...
A ready to be used with Archipelago in a docker-compose file with NGINX. Runs a standard PHP 8.3.x FPM Container on Alpine 3.23 with embeded extras.

### Building the image
```SHELL
$ docker build -t esmero/php-8.3-fpm:1.7.0 .
````
...
- PHP 8.3.31 (lasted on build) with redis extension and D11 Optimizations
...
docker buildx build --platform linux/amd64,linux/arm64 -t esmero/php-8.3-fpm:1.7.0-multiarch . --push
```
...
```docker
docker buildx build --platform linux/amd64,linux/arm64 -t esmero/php-8.3-fpm:1.7.0-dev-multiarch . --push --file Dockerfile.dev
```
